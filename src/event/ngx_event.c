...
ngx_process_events_and_timers(ngx_cycle_t *cycle)
{
    ...
    // 如果设置了timer_resolution参数，timer为-1,也就是epoll_wait只有通过事件触发返回，定时器定时触发epoll_wait返回
    if (ngx_timer_resolution) {
        timer = NGX_TIMER_INFINITE;
        flags = 0;

    } else {
        // 如果没有设置timer_resolution定时器，则每次epoll_wait后跟新时间
    ...
    if (ngx_use_accept_mutex) {
        if (ngx_accept_disabled > 0) {
             ...
             * accept 事件选择放入 ngx_posted_accept_events 队列还是ngx_posted_events 队列
            */
    ...
    */
    ...
    /*
     * 处理客户端的accept事件，调用 ngx_event_t 的 handler 处理对应事件
     * epoll_wait返回后加入到post队列，执行完所有accpet连接事件后，立马释放ngx_accept_mutex锁
     * 其他进程就可以马上获得锁accept客户端连接
     */
