...
/*
 * http请求处理第一步，初始化连接
 * 设置ngx_listening_t的handler，在监听到客户端连接时被调用，具体是在ngx_event_accept函数中
 */
void
ngx_http_init_connection(ngx_connection_t *c)
{
     ...
     * 这时就可调用ngx_http_init_request方法处理请求
     */
    if (rev->ready) {
...
/*
 * 客户端建立连接后，可读事件处理方法
 * 如果在超时范围内又收到客户端请求，则会执行ngx_http_process_request_line
 * 因为handler指向ngx_http_process_request_line
 */
static void
...
ngx_http_process_request_line(ngx_event_t *rev)
{
    ...
    for ( ;; ) {
        ...
        if (rc == NGX_AGAIN) {
            // 读取请求头，数据存储在 header_in 中，n表示执行结果是否正确
        ...
        // 解析请求行，如host，url，method，schema，port等
