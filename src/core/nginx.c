...
// 定义了ngx_core_modules使用的配置指令，同时指定指令的解析函数
...
main(int argc, char *const *argv)
{
    ...
    ngx_log_t        *log;
    ngx_uint_t        i;
    ngx_cycle_t      *cycle, init_cycle; // 核心结构体ngx_cycle_t
    ngx_conf_dump_t  *cd;
    ngx_core_conf_t  *ccf;
    ...
    // 如果命令行参数是-v, 则执行ngx_show_version_info
    if (ngx_show_version) {
        // 打印nginx 版本信息
    ...
    // 初始化各种系统时间
    // 把ngx_cached_time指向cached_time[0],即第一个时间槽，然后调用ngx_time_update()更新时间槽
    ngx_time_init();

#if (NGX_PCRE)
    // 如果添加NGX_PCRE模块，执行此初始化，nginx重写功能依赖此模块
    ngx_regex_init();
#endif
    ...
    // 日志文件初始化，这里是临时日志log，如果有ngx_prefix前缀则日志创建在该前缀路径中，默认是nginx安装目录下的logs/error.log
    ...
    // 将argc argv,environ解析后保存在ngx_argc,ngx_os_argv,ngx_argv,ngx_os_environ中
    ...
    */
    ...
    // 继承socket，继承来的socket放在init_cycle.listening数组（目的平滑升级）
    ...
    // 初始化全局变量cycle（配置文件指定及解析，监听信息，共享内存等）
    ...
    if (ngx_process == NGX_PROCESS_SINGLE) {
        // 进入单进程模式，即只有master进程服务
        ngx_single_process_cycle(cycle);

    } else {
        // 进入master-worker进程模式，master负责管理接收请求及管理worker进程，worker负责处理请求
